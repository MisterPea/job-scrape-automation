...
import { parseWithTimeout } from "./parseController.js";
import { VirtualBrowser } from "./VirtualBrowser.js";
...
export class JobDataRetrieval {
   ...
   */
  private async getNextLink(): Promise<Record<string, string> | false> {
  ...
  async retrieveJob() {
    ...
    if (!link) {
      console.warn("Error: Link is undefined");
      return;
    }
    ...
    try {
      ...
      if (!article) {
        ...
        const errorObj = [
          {
            ...
            location: "pageFetch_parse",
            error: `Couldn't parse link:${link}`,
          },
        ];
        ...
        await this.db.setData(
          ...
          WHERE id=? AND link=?`,
          ["parsed", title, excerpt, siteName, siteData, id, link],
        );
      }
    } catch (err) {
      // General catch
